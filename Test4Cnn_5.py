...
#with open("D:\PythonTest\Test4Cnn\FakedDataSet.txt", "r") as filestream:    
    for line in filestream:
        ...
        rawData.append([float(labelIndex),xAcc,yAcc,zAcc])
        counter+=1

     
        
         
activity1 =[item for item in rawData if item[0] == 1]
...
def createModel():
    ...
    model.add(ks.layers.MaxPooling2D((2, 2), strides=(1,1), padding='same'))
    ...
    model.add(ks.layers.Dense(32,
                              activation=ks.activations.relu,
                              kernel_initializer='he_uniform'))
    


    model.add(ks.layers.Dense(16,
                              activation=ks.activations.relu,
                              kernel_initializer='he_uniform'))
    

    model.add(ks.layers.Dense(8,
                              activation=ks.activations.relu,
                              kernel_initializer='he_uniform'))


    model.add(ks.layers.Dense(6, activation=ks.activations.softmax))
    ...
    model.compile(optimizer=optimizer, loss=ks.losses.SparseCategoricalCrossentropy(), metrics=['accuracy'])
    model.summary()
    return model
